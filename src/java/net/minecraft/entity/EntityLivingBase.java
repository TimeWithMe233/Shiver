...
public abstract class EntityLivingBase extends Entity
{
    ...
    private int getArmSwingAnimationEnd()
    {
        ...
        Animations animationsModule = Shiver.instance.getModuleManager().getModule(Animations.class);

        if(animationsModule.isEnabled()) {
            return this.isPotionActive(Potion.digSpeed) ? 6 - (1 + this.getActivePotionEffect(Potion.digSpeed).getAmplifier()) * 1 : (int) ((this.isPotionActive(Potion.digSlowdown) ? 6 + (1 + this.getActivePotionEffect(Potion.digSlowdown).getAmplifier()) * 2 : 6)  * Minecraft.getMinecraft().timer.timerSpeed);
        }
    ...
    protected void jump()
    {
        ...
        if ((event = new JumpEvent(jumpY, this.rotationYaw, this.isSprinting(), 0)).isCancelled()) {
            return;
        }
        if(this == Minecraft.getMinecraft().thePlayer) {
            Shiver.instance.getEventManager().post(event);
        }
    ...
    public void onLivingUpdate()
    {
        ...
        double motion = ViaMCP.getInstance().getVersion() > 47 ? 0.003D : 0.005D;
