...
import Shiver.handler.client.BlurFilters.GaussianFilter;
...
public class BlurHandler {
    ...
    private void setPostValues() {
        ((Shader)this.shaderGroup.getListShaders().get(0)).getShaderManager().getShaderUniform("BlurDir").set(1.0f, 0.0f);
        this.shaderGroup.loadShaderGroup(this.mc.timer.renderPartialTicks);
        ((Shader)this.shaderGroup.getListShaders().get(0)).getShaderManager().getShaderUniform("BlurDir").set(0.0f, 1.0f);
        this.shaderGroup.loadShaderGroup(this.mc.timer.renderPartialTicks);
    }

    private void setPreValues(float strength) {
        ((Shader)this.shaderGroup.getListShaders().get(1)).getShaderManager().getShaderUniform("Radius").set(0.0f);
        ((Shader)this.shaderGroup.getListShaders().get(1)).getShaderManager().getShaderUniform("BlurDir").set(0.0f, 0.0f);
    ...
    public void blur(float blurStrength, boolean setupOverlayRendering, boolean ignoreModule) {
        if (this.mc.theWorld != null && this.mc.thePlayer != null && !this.oldBlur) {
            GL11.glPushMatrix();
            GL11.glPushMatrix();
            ((Shader)this.blurShaderGroupBuffer.getListShaders().get(0)).getShaderManager().getShaderUniform("Radius").set(blurStrength);
            ((Shader)this.blurShaderGroupBuffer.getListShaders().get(1)).getShaderManager().getShaderUniform("Radius").set(blurStrength);
    ...
    public void bloom(int x, int y, int width, int height, int blurRadius, Color color, boolean ignoreModule) {
        ...
        if (this.shadowCache.containsKey(identifier)) {
            ...
            int texId = TextureUtil.uploadTextureImageAllocate((int)TextureUtil.glGenTextures(), (BufferedImage)blurred, (boolean)true, (boolean)false);
            this.shadowCache.put(identifier, texId);
        }
        GL11.glColor4f((float)1.0f, (float)1.0f, (float)1.0f, (float)1.0f);
        GL11.glBegin((int)7);
        GL11.glTexCoord2f((float)0.0f, (float)0.0f);
        GL11.glVertex2d((double)_X, (double)_Y);
        GL11.glTexCoord2f((float)0.0f, (float)1.0f);
        GL11.glVertex2d((double)_X, (double)(_Y + (float)height));
        GL11.glTexCoord2f((float)1.0f, (float)1.0f);
        GL11.glVertex2d((double)(_X + (float)width), (double)(_Y + (float)height));
        GL11.glTexCoord2f((float)1.0f, (float)0.0f);
        GL11.glVertex2d((double)(_X + (float)width), (double)_Y);
        GL11.glEnd();
