...
implements IMinecraft {
    ...
    public static float getPlayerDirection() {
        float direction = Shiver.instance.getCameraHandler().getYaw();
        boolean forward = Keyboard.isKeyDown((int)MovementUtil.mc.gameSettings.keyBindForward.getKeyCode());
        boolean back = Keyboard.isKeyDown((int)MovementUtil.mc.gameSettings.keyBindBack.getKeyCode());
        boolean left = Keyboard.isKeyDown((int)MovementUtil.mc.gameSettings.keyBindLeft.getKeyCode());
        boolean right = Keyboard.isKeyDown((int)MovementUtil.mc.gameSettings.keyBindRight.getKeyCode());
        if (forward && !back) {
            if (left && !right) {
                direction -= 45.0f;
            } else if (right && !left) {
        ...
        return MathHelper.wrapAngleTo180_float((float)direction);
    }
