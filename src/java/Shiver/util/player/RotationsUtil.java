...
implements IMinecraft {
    ...
    public static boolean raycastEntity(EntityLivingBase target, float yaw, float pitch, float lastYaw, float lastPitch, double reach) {
        ...
        if (entity != null && RotationsUtil.mc.theWorld != null) {
            ...
            Vec3 vec33 = null;
            float f = 1.0f;
            List list = RotationsUtil.mc.theWorld.getEntitiesInAABBexcluding(entity, entity.getEntityBoundingBox().addCoord(vec31.xCoord * d0, vec31.yCoord * d0, vec31.zCoord * d0).expand((double)f, (double)f, (double)f), Predicates.and((Predicate)EntitySelectors.NOT_SPECTATING, (Predicate)new Predicate<Entity>(){
            ...
            if (pointedEntity != null && flag && vec3.distanceTo(vec33) > reach) {
                pointedEntity = null;
                RotationsUtil.mc.objectMouseOver = new MovingObjectPosition(MovingObjectPosition.MovingObjectType.MISS, vec33, (EnumFacing)null, new BlockPos(vec33));
            }
