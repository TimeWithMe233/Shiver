...
extends Module {
    ...
    public void onEnable() {
        ...
        if (this.blockPicker.is("Spoof")) {
            isSpoofing = true;
            Shiver.instance.getSlotSpoofHandler().startSpoofing(Scaffold.mc.thePlayer.inventory.currentItem);
        }
        float yaw1 = MathHelper.wrapAngleTo180_float((float)Scaffold.mc.thePlayer.rotationYaw) + 720.0f;
        switch (this.mode.getMode()) {
            case "Hypixel": {
                if (!this.hypixelSprint.is("Semi") || this.jumpToAvoidSetback.isEnabled() && Scaffold.mc.thePlayer.onGround && !Scaffold.mc.gameSettings.keyBindJump.pressed && Shiver.instance.getAcrimonyClientUtil().getGroundTicks() >= 10) break;
    ...
    private void hypixelScaffold() {
        ...
        if (this.overAir && this.info != null) {
            ...
            if (this.rotationVec3 != null) {
                ...
                if (raytrace != null && raytrace.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK && raytrace.getBlockPos().equals((Object)this.info.pos)) {
            ...
            if (Scaffold.mc.thePlayer.ticksExisted % 2 == 0) {
                PacketUtils.sendPacket((Packet)new C08PacketPlayerBlockPlacement(Scaffold.mc.thePlayer.getHeldItem()));
            }
            this.placing = false;
        }
        Scaffold.mc.gameSettings.keyBindUseItem.pressed = false;
    ...
    private double[] moveFlying(float strafe, float forward, float friction, float yaw) {
        ...
        if (f >= 1.0E-4f) {
            ...
            f = friction / f;
            float f1 = MathHelper.sin((float)(yaw * (float)Math.PI / 180.0f));
            float f2 = MathHelper.cos((float)(yaw * (float)Math.PI / 180.0f));
    ...
    private void godbridgeScaffold() {
        ...
        float yawDiff = Math.abs(MathHelper.wrapAngleTo180_float((float)this.rotations.getYaw()) - MathHelper.wrapAngleTo180_float((float)this.requestedRotationYaw));
        if (yawDiff > 180.0f) {
        ...
        if (this.info != null && this.overAir) {
            ...
            if (raytrace != null && raytrace.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK && raytrace.getBlockPos().equals((Object)this.info.pos) && raytrace.sideHit == this.info.facing) {
                vec3 = raytrace.hitVec;
        ...
        if (Scaffold.mc.thePlayer.onGround) {
            pressSneak = Shiver.instance.getAcrimonyClientUtil().getGroundTicks() <= 2 || this.overAir || this.sneakCounter <= 1;
        } else {
    ...
    private void sneakScaffold() {
        ...
        this.overAir = WorldUtil.isAirOrLiquid(pos);
        this.placing = false;
        float yaw1 = MathHelper.wrapAngleTo180_float((float)MovementUtil.getPlayerDirection()) + 720.0f;
        if (yaw1 % 90.0f < 22.5f || yaw1 % 90.0f > 67.5f) {
            this.diagonally = false;
            this.requestedRotationYaw = yaw1 + 45.0f - (yaw1 + 45.0f) % 90.0f - 135.0f;
        } else {
        ...
        if (this.info != null) {
            if (this.rotationVec3 != null) {
                ...
                if (raytrace != null && raytrace.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK && raytrace.getBlockPos().equals((Object)this.info.pos) && raytrace.sideHit == this.info.facing && this.placeBlock(raytrace.hitVec)) {
                    this.placing = true;
                    this.sneakCounter = 0;
                    this.started = true;
                }
        ...
        boolean pressSneak = false;
        pressSneak = Scaffold.mc.thePlayer.onGround ? Shiver.instance.getAcrimonyClientUtil().getGroundTicks() <= 2 || this.overAir || this.sneakCounter <= 1 : Scaffold.mc.thePlayer.motionY < 0.0;
    ...
    private float getPitch(float yaw) {
        ...
        while (iterator.hasNext()) {
            ...
            if (raytrace == null || raytrace.typeOfHit != MovingObjectPosition.MovingObjectType.BLOCK || !raytrace.getBlockPos().equals((Object)this.info.pos)) continue;
            boolean sameFace = raytrace.sideHit == this.info.facing;
            boolean bl = horizontalFace = raytrace.sideHit != EnumFacing.UP && raytrace.sideHit != EnumFacing.DOWN;
    ...
    private void basicCustomScaffold() {
        ...
        if (this.overAir && this.info != null && allowPlacing && (raytrace = WorldUtil.raytrace(this.rotations.getYaw(), this.rotations.getPitch())) != null && raytrace.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK && raytrace.getBlockPos().equals((Object)this.info.pos) && raytrace.sideHit == this.info.facing) {
    ...
    private void updateRotations(float[] rots, boolean instantRotations, String yawSpeedMode, double minYawSpeed, double maxYawSpeed, double minYawAccel, double maxYawAccel, boolean reduceYawSpeedWhenAlmostDone, double minYawSpeedWhenAlmostDone, double maxYawSpeedWhenAlmostDone, String pitchSpeedMode, double minPitchSpeed, double maxPitchSpeed, double minPitchAccel, double maxPitchAccel, boolean reducePitchSpeedWhenAlmostDone, double minPitchSpeedWhenAlmostDone, double maxPitchSpeedWhenAlmostDone, double minYawChange, double minPitchChange, double rotsRandomisation) {
        ...
        float aaaa = MathHelper.wrapAngleTo180_float((float)yaw);
        float bbbb = MathHelper.wrapAngleTo180_float((float)requestedYaw);
    ...
    private void fixMovement(StrafeEvent event) {
        float value;
        float diff = MathHelper.wrapAngleTo180_float((float)(MathHelper.wrapAngleTo180_float((float)this.rotations.getYaw()) - MathHelper.wrapAngleTo180_float((float)this.getYawDirection()))) + 22.5f;
        if (diff < 0.0f) {
            diff = 360.0f + diff;
        }
            ...
            block48 : switch (this.tower.getMode()) {
                ...
                case "NCP": {
                    ...
                    if (Scaffold.mc.thePlayer.onGround) {
                        ...
                        Scaffold.mc.thePlayer.setPosition(Scaffold.mc.thePlayer.posX, (double)Math.round(Scaffold.mc.thePlayer.posY), Scaffold.mc.thePlayer.posZ);
