...
import Shiver.util.player.PlayerUtil;
...
extends Module {
        ...
        switch (this.mode.getMode()) {
            case "Vulcan": {
                ...
                if (this.ticks == 4) {
                    ...
                    PacketUtils.sendPacketNoEvent((Packet)new C08PacketPlayerBlockPlacement(new BlockPos(Speed.mc.thePlayer.posX + x, Speed.mc.thePlayer.posY - 2.0, Speed.mc.thePlayer.posZ + z), EnumFacing.UP.getIndex(), Speed.mc.thePlayer.inventory.getStackInSlot(Speed.mc.thePlayer.inventory.currentItem), 0.5f, 1.0f, 0.5f));
    ...
    public void onMove(MoveEvent event) {
        ...
        float direction = MathHelper.wrapAngleTo180_float((float)MovementUtil.getPlayerDirection());
        ...
        switch (this.mode.getMode()) {
            case "Fake strafe": {
                ...
                if (this.actualGround) {
                    ...
                    if (!this.wasCollided) {
                        this.motionX -= (double)(MathHelper.sin((float)dir) * 0.2f);
                        this.motionZ += (double)(MathHelper.cos((float)dir) * 0.2f);
                    }
                }
                ...
                if (thing >= 1.0E-4f) {
                    if ((thing = MathHelper.sqrt_float((float)thing)) < 1.0f) {
                        thing = 1.0f;
                    }
                    thing = attributeSpeed / thing;
                    float f1 = MathHelper.sin((float)(direction * (float)Math.PI / 180.0f));
                    float f2 = MathHelper.cos((float)(direction * (float)Math.PI / 180.0f));
