...
import Shiver.handler.packet.DelayedPacket;
...
import Shiver.util.player.PendingVelocity;
...
extends Module {
    ...
    public void onReceive(PacketReceiveEvent event) {
        ...
        if (currentTarget == null) {
            ...
            if (packet2.getEntity((World)Backtrack.mc.getNetHandler().clientWorldController) == currentTarget && this.killauraModule.getDistanceCustomPosition(posX = (double)(x = currentTarget.serverPosX + packet2.getX()) / 32.0, posY = (double)(y = currentTarget.serverPosY + packet2.getY()) / 32.0, posZ = (double)(z = currentTarget.serverPosZ + packet2.getZ()) / 32.0, currentTarget.getEyeHeight()) >= this.minRange.getValue()) {
                event.setCancelled(true);
                this.delayedPackets.add(new DelayedPacket((Packet)packet2));
            }
            ...
            if (packet3.getEntityId() == currentTarget.getEntityId()) {
                ...
                if (this.killauraModule.getDistanceCustomPosition(x, y, z, currentTarget.getEyeHeight()) >= this.minRange.getValue()) {
                    event.setCancelled(true);
                    this.delayedPackets.add(new DelayedPacket((Packet)packet3));
                }
            }
    ...
    public void handleEntityMovement(S14PacketEntity packetIn) {
        Entity entity = packetIn.getEntity((World)Backtrack.mc.getNetHandler().clientWorldController);
        if (entity != null) {
    ...
    public void handleConfirmTransaction(S32PacketConfirmTransaction packetIn) {
        ...
        if (container != null && !packetIn.func_148888_e()) {
            mc.getNetHandler().addToSendQueue((Packet)new C0FPacketConfirmTransaction(packetIn.getWindowId(), packetIn.getActionNumber(), true));
        }
    }
