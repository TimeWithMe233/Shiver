...
import Shiver.util.misc.DeltaTime;
...
extends Module {
    ...
    public void onClientStarted() {
        ...
        this.speedModule = Shiver.instance.getModuleManager().getModule(Speed.class);
        ...
        this.breakerModule = Shiver.instance.getModuleManager().getModule(Breaker.class);
        this.antivoidModule = Shiver.instance.getModuleManager().getModule(Antivoid.class);
        this.velocityModule = Shiver.instance.getModuleManager().getModule(Velocity.class);
    }
    ...
    public void onTick(TickEvent event) {
        ...
        if (d <= d2) {
            ...
            if (this.delayTransactions.isEnabled()) {
                Shiver.instance.getPacketDelayHandler().startDelayingPing(2000L);
            }
            this.hadTarget = true;
        } else {
            this.hadTarget = false;
        ...
        if (attackTick) {
            ...
            if (canAttack) {
                AttackOrder.sendFixedAttack((EntityPlayer)Killaura.mc.thePlayer, (Entity)this.target);
            }
        ...
        if (!this.rotations.is("None") && this.isRotating() && this.moveFix.is("Silent")) {
            float value;
            float diff = MathHelper.wrapAngleTo180_float((float)(MathHelper.wrapAngleTo180_float((float)this.fixedRotations.getYaw()) - MathHelper.wrapAngleTo180_float((float)MovementUtil.getPlayerDirection()))) + 22.5f;
            if (diff < 0.0f) {
                diff = 360.0f + diff;
            }
    ...
    private boolean canBlock() {
        ...
        if (this.autoblock.is("Blink")) {
            ...
            if (this.target != null && !this.whileTargetNotLooking.isEnabled()) {
                ...
                float targetYaw = MathHelper.wrapAngleTo180_float((float)this.target.rotationYaw);
                float diff = Math.abs(MathHelper.wrapAngleTo180_float((float)Killaura.mc.thePlayer.rotationYaw) - targetYaw);
                boolean bl = targetLooking = diff > 90.0f && diff < 270.0f || (double)Killaura.mc.thePlayer.getDistanceToEntity((Entity)this.target) < 1.3;
                if (!targetLooking) {
                    return false;
                }
            }
        ...
        return this.target != null && stack != null && stack.getItem() instanceof ItemSword && (this.whileSpeedEnabled.isEnabled() || !Shiver.instance.getModuleManager().getModule(Speed.class).isEnabled());
    }
    ...
    private void beforeAttackAutoblock(boolean attackTick) {
        ...
        switch (this.autoblock.getMode()) {
            ...
            case "Spoof2": {
                ...
                this.lastSlot = slot;
                Shiver.instance.getSlotSpoofHandler().startSpoofing(slot);
        ...
        switch (this.autoblock.getMode()) {
            ...
            case "Watchdog": {
                if (ViaMCP.getInstance().getVersion() < 47) {
                    PacketUtils.sendPacketNoEvent((Packet)new C08PacketPlayerBlockPlacement(new BlockPos(-1, -1, -1), 255, Killaura.mc.thePlayer.inventory.getCurrentItem(), 0.0f, 0.0f, 0.0f));
                    PacketWrapper useItem = PacketWrapper.create((int)29, null, (UserConnection)((UserConnection)Via.getManager().getConnectionManager().getConnections().iterator().next()));
                    useItem.write((Type)Type.VAR_INT, (Object)1);
                    PacketUtil.sendToServer((PacketWrapper)useItem, Protocol1_8TO1_9.class, (boolean)true, (boolean)true);
                }
                blocking = true;
            }
        }
    ...
    private void releaseBlocking() {
        ...
        if (this.hadTarget && this.autoblock.is("Blink") && !blocking && this.target == null) {
            LogUtil.addChatMessage("Autoblock test : " + Shiver.instance.getPacketBlinkHandler().isBlinking());
        }
        int slot = Killaura.mc.thePlayer.inventory.currentItem;
        if (blocking) {
            switch (this.autoblock.getMode()) {
                ...
                case "Watchdog": {
                    if (ViaMCP.getInstance().getVersion() > 47) break;
                    Killaura.mc.gameSettings.keyBindUseItem.pressed = false;
    ...
    private void getRotations() {
        ...
        if (this.target != null) {
            ...
            switch (this.rotations.getMode()) {
                ...
                case "Smooth": {
                    float yaw1 = rots[0];
                    float currentYaw = MathHelper.wrapAngleTo180_float((float)yaw);
    ...
    public EntityLivingBase findTarget(boolean allowSame, double range) {
        ...
        if (entities != null && entities.size() > 0) {
            switch (this.filter.getMode()) {
                case "Range": {
                    entities.sort(Comparator.comparingDouble(entity -> entity.getDistanceToEntity((Entity)Killaura.mc.thePlayer)));
                    break;
                }
                case "Health": {
    ...
    public boolean canAttack(EntityLivingBase entity, double range) {
        ...
        if ((entity.isInvisible() || entity.isInvisibleToPlayer((EntityPlayer)Killaura.mc.thePlayer)) && !this.invisibles.isEnabled()) {
            return false;
        }
    ...
    public void renderJello(EntityLivingBase e) {
        ...
        for (j = 0; j < 361; ++j) {
            ...
            GL11.glVertex3d((double)(x + Math.cos(Math.toRadians(j)) * size), (double)(y + yOffset), (double)(z - Math.sin(Math.toRadians(j)) * size));
            Killaura.glColor(new Color(255, 255, 255, 10), (int)(!this.down ? 255.0 * height : 255.0 * (1.0 - height)));
            GL11.glVertex3d((double)(x + Math.cos(Math.toRadians(j)) * size), (double)(y + yOffset + (!this.down ? -0.5 * (1.0 - height) : 0.5 * height)), (double)(z - Math.sin(Math.toRadians(j)) * size));
        }
        GL11.glEnd();
        GL11.glBegin((int)2);
        for (j = 0; j < 361; ++j) {
