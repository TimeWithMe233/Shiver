...
import Shiver.util.render.ColorUtil;
                ...
                switch (this.mode.getMode()) {
                    case "Future": {
                        ...
                        if (index == 4) {
                            ...
                            if (this.blurbg.isEnabled()) {
                                Shiver.instance.blurHandler.blur((double)currentX, (double)currentY, (double)totalLength, (double)length, 0.0f);
                            }
                        ...
                        if (this.blurbg.isEnabled()) {
                            Shiver.instance.blurHandler.blur((double)currentX, (double)currentY, (double)length, (double)length, 0.0f);
                        }
                        ...
                        String keyName = Keyboard.getKeyName((int)key.get().getKeyCode());
                    ...
                    case "Blur": {
                        ...
                        if (index == 4) {
                            ...
                            currentY = y + (length + spacing) * 2;
                            totalLength = length * 3 + spacing * 2;
                            Shiver.instance.blurHandler.bloom(currentX, currentY, totalLength, length, ClientTheme.blurradius.getValue(), new Color(0, 0, 0, 150));
                            finalRenderX = currentX;
                            int finalRenderY = currentY;
                            key.updateState(key.get().isKeyDown());
                            DrawUtil.drawRoundedRect(currentX + totalLength - 62, currentY + 10, currentX + totalLength - 6, currentY + 14, 2.0, new Color(246, 246, 246).getRGB());
                            if (!key.isRendered() && key.isAnimDone()) continue block10;
                            key.render(() -> Shiver.instance.blurHandler.bloom(finalRenderX, finalRenderY, totalLength, length, ClientTheme.blurradius.getValue(), this.theme.getColor(1)), finalRenderX, finalRenderY, finalRenderX + totalLength, (float)finalRenderY + (float)length * 0.85f);
                            break;
                        }
                        if (index == 1) {
                            currentX = x;
                        ...
                        Shiver.instance.blurHandler.bloom(currentX, currentY, length, length, ClientTheme.blurradius.getValue(), new Color(0, 0, 0, 150));
                        int finalRenderX2 = currentX;
                        int finalRenderY = currentY;
                        key.updateState(key.get().isKeyDown());
                        if (key.isRendered() || !key.isAnimDone()) {
                            key.render(() -> Shiver.instance.blurHandler.bloom(finalRenderX2, finalRenderY, length, length, ClientTheme.blurradius.getValue(), this.theme.getColor(1)), finalRenderX2, finalRenderY, finalRenderX2 + length, finalRenderY + length);
                        }
                        String keyName = Keyboard.getKeyName((int)key.get().getKeyCode());
                    ...
                    case "Jello": {
                        ...
                        Shiver.instance.blurHandler.bloom(currentX, currentY, length, length, 20, new Color(0, 0, 0, 150));
