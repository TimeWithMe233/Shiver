    ...
    public static List<String> getOnlineConfigList(String repoOwner, String repoName, String token) {
        ...
        try {
            ...
            if (responseCode == 200) {
                ...
                reader.close();
                Gson gson = new Gson();
                for (GitHubFile file : files = (GitHubFile[])gson.fromJson(response.toString(), GitHubFile[].class)) {
    ...
    public static void downloadConfig(String name, String repoOwner, String repoName, String downloadPath, String token) {
        ...
        try {
            ...
            if (responseCode == 200) {
                ...
                reader.close();
                Gson gson = new Gson();
                GitHubFile[] files = (GitHubFile[])gson.fromJson(response.toString(), GitHubFile[].class);
        ...
        try {
            ...
            if (responseCode == 200) {
                ...
                for (GitHubFile file : files = (GitHubFile[])gson.fromJson(response.toString(), GitHubFile[].class)) {
                    ...
                    Shiver.instance.getNotificationHandler().postNotification(new Notification(NotificationType.INFO, "Invalid Config Detected!", "Invalid Config Detected!", 2500L));
                }
                Shiver.instance.getNotificationHandler().postNotification(new Notification(NotificationType.INFO, "Update Config", "Successfully Updated Configs", 2500L));
            } else {
                Shiver.instance.getNotificationHandler().postNotification(new Notification(NotificationType.WARNING, "Update Config", "Failed to update Configs. Response code: " + responseCode, 2500L));
            }
            connection.disconnect();
        }
        catch (IOException e) {
            Shiver.instance.getNotificationHandler().postNotification(new Notification(NotificationType.ERROR, "Update Config", "Error updating Configs: " + e.getMessage(), 2500L));
            e.printStackTrace();
        }
    }
